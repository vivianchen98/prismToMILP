...
def callGroubi(thresholdLower,thresholdUpper,destState, choices,StateNum,choices_sat,InitState,trans,reward,rvalue,c):
    m = Model("example")
    ...
    for i in range(StateNum):
        x_lower[i] = m.addVar(vtype=GRB.CONTINUOUS, name='x%d_lower' % i) #changed to integer from Infinity
        x_upper[i] = m.addVar(vtype=GRB.CONTINUOUS, name='x%d_upper' % i) #changed to integer from Infinity
    m.update()
...
#-------------------------------constraints------------------------------------------------------
    ...
    for (s,a) in choices:
        m.addConstr(x_lower[s] <= c*(1-y[s,a]) + (quicksum(x_lower[t] for s,a,t in trans.select(s,a,'*')) +
                                quicksum(quicksum(rvalue[r,s,a,t] for r,s,a,t in reward.select(r,s,a,'*')) for r in range(0,1))))
        m.addConstr(x_upper[s] >= -c*(1-y[s,a]) + (quicksum(x_upper[t] for s,a,t in trans.select(s,a,'*')) +
                                quicksum(quicksum(rvalue[r,s,a,t] for r,s,a,t in reward.select(r,s,a,'*')) for r in range(0,1))))
    m.update()
...
def generate_lp_script(modelname, thresholdLower, thresholdUpper, destState, objectives, alpha, beta, c):
    ...
    for line in file_trans:
        ...
        if(len(tmp) == 5):
            ...
            trans[(int(tmp[0]),int(a),int(tmp[2]))] = int(tmp[3])
    ...
    for line in file_reward_1:
        ...
        t = int(tmp[2])
        a = trans_dict[(s,t)]
        reward[(0,int(tmp[0]),int(a),int(tmp[2]))] = int(tmp[3])
        rvalue[(0,int(tmp[0]),int(a),int(tmp[2]))] = int(tmp[3])
    ...
    start_time_solve = callGroubi(thresholdLower,thresholdUpper,destState,choices,StateNum,choices_sat,InitState,trans,reward,rvalue,c)
