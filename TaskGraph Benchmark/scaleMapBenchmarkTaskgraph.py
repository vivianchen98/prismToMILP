...
def findDestState():
    destState = 0
    b = open("taskgraph.tra", "r")
    for line in b:
        if "tasks_complete" in line:
            line = line.split(" ")
            destState = int(line[0])

    #destState = [3017,3018,3019,3020,3021,3022,3023,3024,3025] #K=1
    destState = [7335,7336,7337,7338,7339,7340,7341,7342,7343,7344,7345,7346,7347,7348] #K=2
...
def main():
    ...
    os.system("sh prism -javastack 1g taskgraph.prism taskgraphpropertiesfile.pctl -const K=2 -exportmodel .tra,sta,lab -exporttransrewards taskgraph.rew -exportresults resultsLog.txt")
    ...
    countStates = len(open("taskgraph.sta").readlines(  ))-1
    countTransitions = len(open("taskgraph.tra").readlines(  ))-1
    output.write("MDP States:"+str(countStates)+"\n")
    output.write("MDP Transitions:"+str(countTransitions)+"\n")

    print("Generating MILP")
    start_time_overall,start_time_solve = generate_lp_script("taskgraph",thresholdLower,thresholdUpper,destState,objectives,alpha,beta,c)
