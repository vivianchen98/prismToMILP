...
def findDestState():
    ...
    destState = []
    for i in range(906994,907993,1):
        destState.append(i)
...
def main():
    ...
    os.system("sh prism -javastack 1g teamform5.prism teamform5properties.pctl -const K=1 -exportmodel .tra,sta,lab -exporttransrewards teamform5.rew -exportresults resultsLog.txt")
    ...
    countStates = len(open("teamform5.sta").readlines(  ))-1
    countTransitions = len(open("teamform5.tra").readlines(  ))-1
    output.write("MDP States:"+str(countStates)+"\n")
    output.write("MDP Transitions:"+str(countTransitions)+"\n")

    print("Generating MILP")
    start_time_overall,start_time_solve = generate_lp_script("teamform5",thresholdLower,thresholdUpper,destState,objectives,alpha,beta,c)
